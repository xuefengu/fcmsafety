...
<!-- badges: end -->
  ...
  The goal of fcmsafety is to evaluate the safety of compounds from food contact 
  materials. It takes into account the toxicity data from ECHA's Substances of 
  Very High Concern (SVHC), Carcinogenic, Mutagenic, and Reprotoxic (CMR) from 
  the Classification, Labelling, and Packaging (CLP) regulation, Carcinogenic 
  substances from IARC, Endocrine Disrupting Chemicals (EDC) from The International 
  Panel on Chemical Pollution (IPCP) commissioned by UN Environment, Specific 
  Migration Limit (SML) from both EU 10/2011 and China GB 9685 regulation. In 
  addition, flavonet and classifire information retrieval are integrates well.

## Installation

You can install the development version of fcmsafety from [GitHub](https://github.com/) with:
...
You do not have the "rio" and "dplyr" package installed, please install it first. 
...
Please name your file in English but not Chinese. There should be at least two 
column in your data. One contains chemical names in English, another one includes
CAS number. The program will first use CAS and then chemical name to retrieve 
meta data from Pubchem. A good chemical name is always favorable but CAS number
is not mandatory. 
```{r}
...
Please specify which column contains CAS number by the "cas_col" argument, and 
chemical name by the "name_col" argument. If you have InChIKey column, you can 
also specify it by "inchikey_col".
```{r}
...
For compounds that do not present in any of the databases we consider, we can use 
Toxtree (http://toxtree.sourceforge.net/) to predict its Cramer rules level. We 
now can export a file that can be used by Toxtree for batch processing. 
```{r}
