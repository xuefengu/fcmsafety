...
1. extract_cid() now supports any of the following keys, "InChIKey", "CAS", 
...
2. Remove warnings when load_databases().

### Others

1. Deprecate evaluate_compound() function and create a new one named assign_toxicity().
2. Retrieval CAS number and flavornet information in the extract_meta() function.
3. Create a new function named extract_claasyfire() for classyfire information retrieval.
...
SMILES retrieved for these compounds using \code{extract_meta}. In this case,
you can use \code{assign_meta} function. However, a *.txt file containing Name 
...
(case-insensitive) of these molecules and extract SMILES using \code{combine_mol2sdf()}
...
"D:/data/mydata_toxtree_results.csv"). 
```{r}
data_complete <- data %>% 
